...
class NASA_Exoplanet_Archive_CompositeDefault(cat.CatalogComposite):
    ...
    def load_API_table(self, force_load=False):
        try:
            if force_load:
            ...
            logger.warning('Accessing NASA Exoplanet Archive')
            service = vo.dal.TAPService("https://exoplanetarchive.ipac.caltech.edu/TAP/") 
            logger.warning('Table accessed')
    ...
    def _rename_columns(self):
        logger.info('Renaming keys to common names')
        self.table = self.table.rename(columns=_MAPPER)
        return
    
    def _drop_columns(self):
        logger.info('Dropping unrelated keys.')
